...
public static void spdata2() throws  Exception {
Scanner s=new Scanner(System.in);
	String url="jdbc:mysql://localhost:3306/Studentds";
	String username="root";
	String password="Vicky@2611";
	...
	int id=s.nextInt();
	Connection con=DriverManager.getConnection(url, username, password);
	CallableStatement cst=con.prepareCall("{call GetstNamebyId(?,?)}");
cst.setInt(1, id);
cst.registerOutParameter(2, Types.VARCHAR);
int rows=cst.executeUpdate();
System.out.println("your name is: "+cst.getString(2));
con.close();
}



public static void spdata3() throws  Exception {
Scanner s=new Scanner(System.in);
	String url="jdbc:mysql://localhost:3306/Studentds";
	String username="root";
	String password="Vicky@2611";
	...
	int id=s.nextInt();
	Connection con=DriverManager.getConnection(url, username, password);
	CallableStatement cst=con.prepareCall("{call GetschoolNamebyId(?,?)}");
cst.setInt(1, id);
cst.registerOutParameter(2, Types.VARCHAR);
int rows=cst.executeUpdate();
System.out.println("Your school name is: "+cst.getString(2));
con.close();
}
}